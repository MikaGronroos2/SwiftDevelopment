...
struct Eduskunta: View {
    ...
    @State private var constituencies: Set<String> = []
    @State private var edustajatState: [Edustaja] = []

    func getAllConstituencies()  {
        edustajat.forEach { edustaja in
            constituencies.insert(edustaja.constituency)
        }
    ...
    var body: some View {
        NavigationView {
            VStack {
                ...
                NavigationLink(destination: MapView(constituencies: $constituencies, edustajat: $edustajatState)) {
                    Text("Open Map to view all Constituencies")
                }
